...
- Queues SMS for review in the TxtCow app before sending
- Supports optional automatic sending when you want a faster flow
...
- Review-and-approve flow that reduces wrong sends
...
4. Activate **TxtCow SMS** in WordPress.
5. Open **Settings -> TxtCow SMS** and paste your TxtCow API key.
6. Configure notification options. Automatic sending is optional; review-before-send is the default workflow.
7. If you need higher usage, contact support at [support@txtcow.com](mailto:support@txtcow.com).
...
- 앱에서 전송 전에 검토하고 승인하는 흐름이라 오발송을 줄일 수 있음
- 발송 흐름을 직접 통제 가능
- 수신 거부 번호를 중앙에서 관리
- QR 코드로 빠르게 디바이스 연결
...
4. WordPress에서 **TxtCow SMS**를 활성화합니다.
5. **설정 -> TxtCow SMS**로 이동해 TxtCow API 키를 입력합니다.
6. 알림 옵션을 설정합니다. 자동 전송은 선택 옵션이며, 기본 흐름은 앱에서 검토 후 승인입니다.
7. 더 많은 사용량이 필요하면 [support@txtcow.com](mailto:support@txtcow.com)으로 문의하세요.
